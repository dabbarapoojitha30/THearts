...
if(!isAdmin){
    ...
    document.querySelectorAll('#echoForm input, #echoForm select, #echoForm textarea, #echoForm button[type="submit"]').forEach(el => el.style.display='none');
}

// ------------------- LOGOUT BUTTON -------------------
document.getElementById('logoutBtn')?.addEventListener('click', () => {
    localStorage.removeItem('currentUser'); // clear login info
...
if(downloadBtn){
    downloadBtn.addEventListener("click", async () => {
        ...
        if(!patientId || !name){ alert("Patient ID and Name are required"); return; }
        ...
        try {
            ...
            a.href = url;
            a.download = `${patientId}-${name.replace(/[^a-z0-9]/gi,'_')}.pdf`;
